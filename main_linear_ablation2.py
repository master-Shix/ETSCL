...
from networks.resnet_big import SupConResNet, LinearClassifier,MultiLinearClassfier2
    ...
    for epoch in range(1, opt.epochs + 1):
       ...
       # print('Train epoch {}, total time {:.2f}, accuracy:{:.2f}'.format(epoch, time2 - time1, acc=acc.item() if hasattr(acc, 'item') else acc))
        ...
        if opt.classes == "all":
            ...
            loss, val_acc,kappa_val = validate_two(val_loader, model_fundus,model_oct,classifier, criterion, opt)
        else:
            loss, val_acc = validate(val_loader, model, classifier, criterion, opt)
        if val_acc > best_acc or kappa_val > best_kappa:
            best_acc = val_acc
            best_kapa = kappa_val
            opt.save_folder = '/root/autodl-tmp/SupContrast/save/SupCon/best_mode/fund_oct_scl_soft'
            ...
            save_file_classifier = os.path.join(opt.save_folder, 'classifier',
                                                str(best_acc) + str(best_kapa) + 'last.pth')
