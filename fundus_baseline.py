...
class Model(nn.Module):
    def __init__(self):
        ...
        self.fundus_branch = resnet34(pretrained=True)
        #self.oct_branch = resnet34(pretrained=True)
        self.fundus_branch.fc = nn.Identity()  # remove final fc
        #self.oct_branch.fc = nn.Identity()  # remove final fc
        self.decision_branch = nn.Linear(512, 3)  # ResNet34 use basic block, expansion = 1
        ...
        for data in train_dataloader:
            ...
            if iter % eval_interval == 0:
                ...
                if avg_kappa >= best_kappa:
                    ...
                    if not os.path.exists("best_model_1"):  
                        os.makedirs("best_model_1") 
                    torch.save(model.state_dict(),
                               os.path.join("best_model_1", 'model.pth'))
                model.train()
...
best_model_path = "./best_model_0/model.pth"
model = Model().cuda()
...
print("The final kappa of fundus only is:")
print(avg_kappa2)