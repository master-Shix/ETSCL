...
from main_ce import set_loader,save_model
...
def parse_option():
    ...
    parser.add_argument('--learning_rate', type=float, default=0.002,
                        help='learning rate')
    parser.add_argument('--lr_decay_epochs', type=str, default='20,40,60,80,100',
...
def train_all(train_loader, model_fundus,model_oct,model_vessel ,classifier, criterion, optimizer, epoch, opt):
    ...
    for idx, (oct,fundus,vessel, labels) in enumerate(train_loader):
        ...
        output_oct, output_fundus, output_vessel, output_combined = classifier(oct_features.detach(),
                                                                               fundus_features.detach(),
                                                                               vessel_features.detach())
       # pdb.set_trace()
        #loss = criterion(output, labels)
...
def validate_all(val_loader, model_fundus,model_oct, model_vessel,classifier, criterion, opt):
    ...
    # pdb.set_trace()
    # print('Test Acc_top1',top1.avg)
    return losses.avg, top1.avg, kappa.avg
...
def main():
    ...
    for epoch in range(1, opt.epochs + 1):
       ...
       # print('Train epoch {}, total time {:.2f}, accuracy:{:.2f}'.format(epoch, time2 - time1, acc=acc.item() if hasattr(acc, 'item') else acc))
        ...
        # eval for one epoch
        if opt.classes == "all":
            loss, val_acc,kappa_val = validate_all(val_loader, model_fundus,model_oct, model_vessel,classifier, criterion, opt)
        else:
            loss, val_acc = validate(val_loader, model, classifier, criterion, opt)
        if val_acc > best_acc or kappa_val> best_kappa:
            best_acc = val_acc
            best_kapa=kappa_val
            opt.save_folder = '/root/autodl-tmp/SupContrast/save/SupCon/best_mode/3_scl_de2'
            save_file_fundus = os.path.join(opt.save_folder, 'fundus', str(best_acc)+str(best_kapa) + 'last.pth')
            save_file_oct = os.path.join(opt.save_folder, 'oct', str(best_acc) + str(best_kapa)+'last.pth')
            save_file_vessel = os.path.join(opt.save_folder, 'vessel', str(best_acc) +str(best_kapa)+ 'last.pth')
            save_file_classifier = os.path.join(opt.save_folder, 'classifier', str(best_acc) +str(best_kapa)+ 'last.pth')
